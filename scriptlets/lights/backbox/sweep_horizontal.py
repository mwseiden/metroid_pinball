...
class SweepHorizontal(DynamicBackBoxShow):
    ...
    def animate(self):
        ...
        if self.frame % self.speed == 0:
            ...
            self.strips[5].set_all_colors(self.color if self.current_sweep == 5 else self.OFF_COLOR)
            self.strips[6].set_all_colors(self.color if self.current_sweep == 5 else self.OFF_COLOR)
            self.strips[7].set_all_colors(self.color if self.current_sweep == 5 else self.OFF_COLOR)
            self.strips[8].set_all_colors(self.color if self.current_sweep == 4 else self.OFF_COLOR)
            self.strips[9].set_all_colors(self.color if self.current_sweep == 3 else self.OFF_COLOR)
